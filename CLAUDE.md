...
- **Referral question moved to the signup form**: "How did you hear about Booki?" used to be Question 4 of the onboarding questionnaire, which is reached **only** from the organizer-creation branch of `detectUserRole`. Members joining by invite were never asked, so `referral_source` was permanently NULL for most of the user base. It is now a field on the signup form, so every signup answers it. There is no session at signup, so the answer rides as `user_metadata` on `auth.signUp` and `recordReferral()` in `dashboard.js` persists it on first authenticated load — metadata travels with the account, so it survives the user verifying in a different browser, which localStorage would not. `onboarding_responses` now has two writers, so migration 054 adds `UNIQUE(auth_user_id)` and both use upsert: without it, `attribution_overview` LEFT JOINs the table without aggregating and a duplicate row silently DOUBLES that user in every /admin tally. `completed` now defaults FALSE, because a signup-form row is not a finished questionnaire. **Deploy order: apply 054 before pushing** — a PostgREST upsert whose `onConflict` column has no unique index raises 42P10, and `recordReferral` swallows it.
