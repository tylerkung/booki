...
- **balldontlie's MLB payload differs from NFL/NBA in ways that fail silently**: MLB calls the road side `away_team`; NFL and NBA call it `visitor_team`. The game matcher read only `visitor_team?.id`, so for MLB it was `undefined`, the filter excluded every game, and the run reported "no balldontlie game for these teams" — a mapping problem that was really a schema one. `roadTeamId()` now accepts either.
- **MLB series break ±1-day game matching**: the matcher searches the event date ±1 (a 7pm ET game is next-day UTC), which in baseball catches the whole three-game series between the same two teams and returns "2 candidate games; refusing to guess". It now disambiguates by first pitch — nearest start time wins — and still refuses when two candidates are within an hour, since a real doubleheader is genuinely ambiguous. NFL never hit this because teams play weekly.
- **Resolve subjects from the ROSTER, not one name at a time**: the original path asked balldontlie for each prop subject by name — about twenty calls per MLB game — and the rate limit answered 429 to half, surfacing as `balldontlie has no player named "Nico Hoerner"` for a player the API returns instantly on its own. One `team_ids[]` call per game fetches both rosters and matching happens locally: resolution went from 10/20 to **20/20**. `bdlFetch` also retries 429 with backoff, and every failure now says "rate limited" rather than "no such player" — conflating the two sends you hunting a name-matching bug that does not exist.
...
- **Migration atomicity in the Supabase SQL editor is NOT something to rely on either way**: migration 046 failed with its `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` and `CREATE POLICY` already committed while its closing assertion raised — a partially-applied migration. Migration 057 then failed on a `CREATE OR REPLACE FUNCTION` and rolled back **whole**, leaving not even the `ALTER TABLE ADD COLUMN` that preceded it (verified: the column did not exist afterwards). So the behaviour varies by failure mode, and a `DO $$ ... RAISE EXCEPTION` block at the end does not reliably undo what came before it. **Write every migration so a partial application is harmless** — `IF NOT EXISTS`, `DROP ... IF EXISTS` then `CREATE` — so a re-run after a failure is always safe, and check the resulting state rather than assuming which way it went.
- **`CREATE OR REPLACE FUNCTION` cannot change a return type**: adding an OUT column to a `RETURNS TABLE (...)` function raises `42P13: cannot change return type of existing function`. It needs an explicit `DROP FUNCTION IF EXISTS name(argtypes);` first. This bit migration 057 when `get_event_player_props` gained `subject_sport` and `bettable`.
